...
当前活跃方向：
- **E8-004-S6（Mencius）**：rate-limit-aware key selection 的 precise TPM estimate contract。
...
- **E8-004-S6，进行中，owner Mencius**：precise TPM estimate contract。原因：E8 runtime DB reservation 与 live smoke 入口已落地，但 `[x]` 仍需要 rate-limit key selection 对 token-per-minute reservation 有稳定、可解释、不过度乐观的估算 contract。范围：`crates/routing/**`、routing fixtures/tests；不改 Gateway/Control Plane/UI。验收：routing pure planner 对 prompt/completion/token estimate 的 TPM reservation 选择有 contract fixture；缺 usage/tokenizer 数据时使用明确保守 fallback；相关 routing tests 通过。
