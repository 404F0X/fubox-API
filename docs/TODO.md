...
- **E11-007-S11 Backend（Goodall）**：ledger adjustment execute OpenAPI/acceptance contract 同步。
...
- **E11-007-S11，进行中，owner Goodall**：ledger adjustment execute OpenAPI/acceptance contract 同步。原因：E11-S7/E9-S7 已让 UI 和 Control Plane 使用真实 execute response 与 `ledger_executor_summary` 边界，但 OpenAPI skeleton/acceptance text 需要同步最新 applied/idempotent/executor-summary/secret-safe 字段，避免客户端按旧 contract 生成。范围：`examples/openapi_admin_skeleton.yaml`、必要的 `TEST_AND_ACCEPTANCE.md`；不改 Gateway/Control Plane/Admin UI/routing/billing-ledger 代码。验收：OpenAPI 描述 `mode=execute` applied/idempotent response、`ledger_executor_summary_contract`/`ledger_executor_summary`、dedupe material omitted、raw metadata/credential 不回显；必要文档指向对应 contract；OpenAPI/yaml diff check 通过。
