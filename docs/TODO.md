...
- **E13-005-S12（Pauli）**：Prompt Protection Postgres proof test/release opt-in gate wiring。
...
- **E13-005-S12，进行中，owner Pauli**：Prompt Protection Postgres proof test/release opt-in gate wiring。原因：S11 已提供独立脚本，但开发/CI 需要统一入口运行默认 contract/preflight，并保留 live proof 显式 opt-in，避免手动脚本与 test/release gates 漂移。范围：`scripts/**`、`docs/**`、必要 `TEST_AND_ACCEPTANCE.md`；不改 Gateway/Control Plane/Admin UI/routing 代码。验收：`scripts/test.ps1` 或 release smoke gate 有明确 Prompt Protection Postgres proof contract-only/default 入口；live flag/env 才进入 `-Live`；外部 blocker exit `2` 不被误报为默认失败；release/test 文档说明可关闭与不可关闭缺口；相关脚本默认检查和 diff check 通过。
