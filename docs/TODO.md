...
- **E11-007-S14 Backend（Goodall）**：ledger execute OpenAPI semantic validator/client generation runbook。
...
- **E11-007-S14，进行中，owner Goodall**：ledger execute OpenAPI semantic validator/client generation runbook。原因：S13 已提供轻量默认 gate，但完整 OpenAPI semantic validator 和 client generation 仍是独立验收项，需要明确工具、命令、外部依赖、失败解释和关闭条件。范围：`docs/**`、必要 `TEST_AND_ACCEPTANCE.md`；不改 Gateway/Control Plane/Admin UI/routing/billing-ledger/scripts。验收：文档列出推荐 validator/client-generation 命令、依赖安装方式、离线/缺工具 blocker 语义、需要检查的 generated client contract、通过后可关闭的 E11 OpenAPI 缺口，以及不能关闭的 live Postgres/UI/audit 缺口；diff check 通过。
