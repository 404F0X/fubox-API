...
当前活跃方向：
- **E8-004-S5（Mencius）**：将 rate-limit reservation live smoke 接入现有测试入口的 opt-in/preflight 流程。
...
- **E8-004-S5，进行中，owner Mencius**：rate-limit reservation live smoke 测试入口接入。原因：S4 已新增可运行 harness，但开发/CI 需要清晰 opt-in 入口和 Docker/Postgres preflight，避免 live smoke 被误当成普通 contract test 或因环境缺失造成假失败。范围：`scripts/**`、必要的测试/验收文档；不改 Gateway 生产逻辑。验收：现有测试入口能发现/调用 dry-run 或 opt-in live smoke；Docker/Postgres 不可用时输出明确阻塞；默认非 live 流程不被破坏；相关 script dry-run/check 通过。
