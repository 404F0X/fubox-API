...
- **E13-005-S10（Pauli）**：Prompt Protection provider_attempts Postgres proof runbook。
...
- **E13-005-S10，进行中，owner Pauli**：Prompt Protection provider_attempts Postgres proof runbook。原因：S6-S9 已用 handler-level spies 覆盖主要 Gateway surfaces，但最终 no-side-effect 证据仍缺真实 Postgres `provider_attempts` 表计数或 live/integration runbook。范围：`docs/**`、必要 `TEST_AND_ACCEPTANCE.md`；不改 Gateway/Control Plane/Admin UI/routing/scripts。验收：文档列出真实 DB 验证前置条件、命令/环境、要覆盖的 endpoints、expected provider_attempts count=0、request log hash-only、blocked exit semantics；明确该 runbook 通过后可关闭的 E13 剩余缺口和不能关闭的 UI/audit 可视化缺口。
