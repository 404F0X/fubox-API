...
- **E11-007-S9，进行中，owner Goodall**：ledger execute live smoke runbook/acceptance blocker 文档。原因：S6/S8 已有 live harness 和 opt-in gate，但最终验收还需要明确操作手册和阻塞条件，方便有 Docker/Postgres 环境的人复跑并关闭 live 缺口。范围：`docs/**`、`TEST_AND_ACCEPTANCE.md`；不改 Gateway/Control Plane/Admin UI/billing-ledger/routing。验收：文档包含 contract-only 默认命令、live opt-in 命令、所需 compose services、预期 blocker exit code、通过后可关闭的 TODO 缺口。
