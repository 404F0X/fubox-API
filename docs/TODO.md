...
- **E9-004-S6（Confucius）**：billing-ledger SQLx executor runtime wiring readiness。
- **E11-007-S8 Backend（Goodall）**：Control Plane ledger adjustment execute smoke 接入 test/release opt-in gate。
- **E13-005-S6 Gateway（Pauli，已完成待合并）**：Prompt Protection reject 的 no-side-effect 回归覆盖已落地。
...
- **E9-004-S6，进行中，owner Confucius**：billing-ledger SQLx executor runtime wiring readiness。原因：S4/S5 已固定 SQLx statement/schema/live preflight，但后续接 Control Plane/Gateway 前还需要 crate 内清晰证明 executor API 的事务结果、row-count error、secret-safe output 能作为 runtime writer 边界复用。范围：`crates/billing-ledger/**`、billing fixtures；不改 scripts/Gateway/Control Plane/UI。验收：新增或强化 executor-facing contract/test，覆盖 transaction result summary、row-count mismatch/refusal、operation key secret-safe debug/output；默认 tests 和 `postgres-sqlx` feature tests 通过。
