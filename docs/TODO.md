...
- **E11-007-S13 Backend（Goodall）**：ledger execute OpenAPI contract validation gate。
...
- **E11-007-S13，进行中，owner Goodall**：ledger execute OpenAPI contract validation gate。原因：S11 已同步 OpenAPI skeleton，但目前只有 YAML parse 和人工 diff；需要一个轻量可运行检查锁定 ledger adjustment execute/executor summary refs，避免后续 schema drift。范围：`scripts/**`、`examples/openapi_admin_skeleton.yaml`、必要验收文档；不改 Gateway/Control Plane/Admin UI/routing/billing-ledger 代码。验收：新增或强化脚本/检查能 parse OpenAPI YAML，并断言 `/admin/ledger/adjustments/dry-run`、execute applied/idempotent schema、execute_contract refusal schema、executor summary/refusal/rollback refs 和 secret-safe omission markers 存在；默认不依赖外部服务；diff check 通过。
