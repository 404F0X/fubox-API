...
- **E11-007-S8 Backend（Goodall）**：Control Plane ledger adjustment execute smoke 接入 test/release opt-in gate。
...
- **E11-007-S8，进行中，owner Goodall**：Control Plane ledger adjustment execute smoke 测试入口接入。原因：S6 已新增可运行 harness，但开发/CI 需要明确默认 contract-only 与 opt-in live gate，避免真实 DB 不可用时造成默认失败。范围：`scripts/**`、必要验收文档；不改 Gateway/Admin UI/billing-ledger。验收：test/release 入口能默认跑 contract-only；显式 flag/env 才跑 live；Docker/Postgres/control-plane 不可用时明确阻塞；相关 script checks 通过。
