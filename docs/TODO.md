...
- **E11-007-S5 UI（Godel）**：推进 Billing/Price 页面 execute-contract v2 的 UI 可验收呈现与错误流。
- **E13-005-S3 Gateway（Pauli）**：推进 Prompt Protection 的 Gateway endpoint 覆盖，从 embeddings 扩展到 remaining native/provider 路径。
...
- **E11-007-S5，进行中，owner Godel**：Admin UI execute-contract v2 呈现与错误流。原因：后端真实执行落地前，UI 需要把 execute contract v2 的预检、dedupe、事务/audit 摘要和阻塞原因展示清楚，避免用户把 plan-only 当成已入账。范围：`web/admin-ui/**`；不改后端。验收：UI 明确区分 dry-run/execute-preflight/blocked/success；错误不泄露 secret；相关 UI 测试和 bundle check 覆盖。
- **E13-005-S3，进行中，owner Pauli**：Prompt Protection Gateway endpoint 覆盖。原因：embeddings 已完成，但 Prompt Protection 最终验收需要覆盖 remaining native/provider paths，并保持 billing/log/audit 的 sanitized prompt_protection signals。范围：`apps/gateway/**`、Gateway fixtures；不改 Admin UI/Control Plane。验收：至少一个 remaining endpoint/provider path 的 protection ordering、block response、log metadata sanitizer 和 regression fixture 固定；未覆盖路径列入剩余缺口。
