...
- **E13-005-S9 Gateway（Pauli）**：Prompt Protection no-side-effect regression 扩展到 Gemini native path。
...
- **E13-005-S9，进行中，owner Pauli**：Prompt Protection no-side-effect regression 扩展到 Gemini native path。原因：S8 已补 Anthropic native handler-level spy，E13 Gateway surface 仍剩 Gemini native 只有 preflight ordering/unit 证据。范围：`apps/gateway/**`、Gateway prompt-protection fixtures；不改 Admin UI/Control Plane/billing/routing/scripts/docs。验收：Gemini native reject 通过 handler-level spy 断言 provider_attempt/provider key/upstream count 为 0；request log hash-only 且 metadata secret-safe；runtime fixture 增加 Gemini native no-side-effect contract；targeted Gateway tests 通过。
