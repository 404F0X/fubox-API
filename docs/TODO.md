...
- **E9-004-S7，进行中，owner Confucius**：Control Plane execute writer 与 billing-ledger executor summary 集成准备。原因：E11-S4 已有 Control Plane 本地最小 SQL writer，E9-S6 已有 billing-ledger executor summary；下一步需要锁定两者之间可迁移的 response/audit summary 边界，避免后续 runtime writer 接入时改变 UI/API 安全输出。范围：`apps/control-plane/**`、control-plane billing fixtures；不改 scripts/Gateway/Admin UI/routing。验收：Control Plane execute response 或 fixture 明确包含/预留 billing-ledger executor summary 兼容字段，保持 secret-safe；targeted control-plane billing/ledger tests 通过。
