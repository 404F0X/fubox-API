...
- **E9-004-S9（Confucius）**：Control Plane 到 billing-ledger runtime writer 的纯映射 contract。
...
- **E9-004-S9，进行中，owner Confucius**：Control Plane 到 billing-ledger runtime writer 的纯映射 contract。原因：S7/S8 已固定 response/refusal summary 形状，下一步需要先用 DB-free contract 锁定 Control Plane validated ledger adjustment plan 到 billing-ledger runtime writer operation/summary 的字段映射，再做真实 writer 替换。范围：`apps/control-plane/**`、必要的 `crates/billing-ledger/**` contract/tests 与 fixtures；不改 Gateway/Admin UI/routing/scripts。验收：新增或强化纯函数/test，覆盖 adjust/refund applied/idempotent/refusal 映射到 billing-ledger executor operation/outcome/statement summary；保持 operation key/raw metadata/credential omitted；不要求 live DB；targeted control-plane 与 billing-ledger tests 通过。
