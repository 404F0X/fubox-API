...
## 11. Adapter Conformance Harness

本地 adapter fixture 结构检查使用：

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\adapter_conformance.ps1
cargo test -p ai-gateway-adapters --all-targets
```

Harness 枚举 `tests/fixtures/adapters/{openai,anthropic,gemini}`，检查 fixture name 全局唯一、valid/error/stream 分类齐备、必要 contract 字段存在，并阻止 secret-like 示例值进入 fixtures。
