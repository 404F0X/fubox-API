...
P0 不是功能越多越好，而是要能支持站长自托管实例和小规模用户池灰度使用。优先级排序如下：
...
2026-06-07 产品口径更新：当前 MVP 按 New API 类型收敛，不把企业化角色作为第一交付目标。复杂企业治理、SSO、专职安全管理员、财务运营工作台进入 P1/P2；P0 先服务站长和 API 用户。

| 角色 | 目标 | 关键功能 |
|---|---|---|
| 站长/管理员 | 部署平台，配置上游、渠道、模型、价格、兑换码、用户和日志 | Admin UI、Provider/Channel、Model、Price、Voucher、Logs |
| 用户/开发者 | 注册登录，兑换额度，创建 API Key，调用统一 endpoint，查看余额和用量 | User Portal、OpenAI-compatible API、API Key、Wallet、Usage |
| 上游供应商 | 提供真实模型能力和供应商 Key | Provider Key、Channel、Health、Routing |

AxonHub 参考点保留在体验层：Any SDK -> Any Model、模型别名/Profile、Trace、协议兼容和排障解释。

## 3. P0：生产初版

P0 目标：可在站长自托管/多用户 API 分发场景中灰度上线，替代 New API 的核心链路，同时吸收 AxonHub 的协议网关体验。
...
| Enterprise Developer Portal | 模型申请、额度申请、服务目录、审批；这是企业/团队后置门户，不是当前 New API User Portal |
