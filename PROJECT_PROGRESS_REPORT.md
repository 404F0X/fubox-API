...
| M3 路由和流式稳定 | 进行中 | Retry/fallback、stream finalizer、terminal validation、TTFT、usage settle 已有核心路径；本轮接入配置化 attempts/timeout/stream idle timeout。 |
| M4 账务和观测 | 进行中 | Price/preauth/settle/ledger summary/reconciliation/metrics 已有切片；reserve/refund runtime、强一致余额和 scheduler 待补。 |
| M5 管理后台 | 进行中 | 登录、导航/RBAC、Provider/Channel、Models、Virtual Keys、Request/Trace、Health、Billing 页面均有 MVP。 |
...
- E1 身份/RBAC/审计：登录、session、RBAC、audit read/write 多数切片完成；Virtual Key 写路径本轮补事务化 audit；OIDC 仍是安全草案/plan-only。
...
- E8 Routing/Fallback/Health：routing core、priority/weight、health/rate-limit 纯函数、fallback engine 已有；本轮接入 configured max attempts；Gateway DB 候选生成、rate counters、trace affinity DB 记忆待补。
...
- E13 Security/Compliance：secret masking、payload redaction、provider encryption、offline supply-chain artifacts、prompt protection non-stream preflight 已有；本轮补 body-limit 前置、CORS allowlist、endpoint SSRF guard、redirect 禁用和 readiness 脱敏；联网漏洞/镜像扫描、digest pinning、stream prompt protection 待补。
...
- Audit 仍有 provider/channel/profile/model/model association 等写路径是业务写入后补 audit；已优先修 provider_key、price_version、virtual_key。
